...
校验方式：解包后运行 `sha256sum -c SHA256SUMS`（逐文件校验）；`SHA256SUMS` 由交付方在打包时生成并随包分发。kit 整包与解压内容树的数字见 `device-test-kit` release 说明的「## Integrity」小节（`workload-latest` 镜像同值）或 `.tar.gz.sha256` sidecar；本轮实测 tar **196,220,486 B** / `091dcc56…`、树 **`0a7a3215…`**、sidecar `d7efd251…`（详见 `docs/plans/2026-09-26-ohos-tester-handoff-kit28.md`）。当前发布为 **kit #28**（2026-09-26；R2：**Map 覆盖层**（`ARKTS_SDK_FLAVOR=harmony` 壳 + AGC 地图 AppKey；默认 flavor 下 `IsSupported` 可为 true 而 `IsOverlayAvailable=false`，show/hide/区域/标记等 overlay 调用**降级不抛**）+ **LiveView 特性探测**（无 Kit/权益时 `IsSupported=false`、Start/Update/Stop `Unavailable` 不抛）+ **壳 `start_app` AOT 启动桥**（`lib<stem>.so` → `openharmony_app_main`，日志 `aot=1`，失败 `aot=0` 回退 JIT）+ **解释器实验资产**（`<files>/interp.txt` → `DOTNET_InterpMode`，日志 `interp=3 source=file`）；宿主导出契约 **134/134**；ui/shell abc → **264,136 B**（headless 18,532 B）；交互门禁 **334/floor 314**）：包内 5 个 hap 均带 `"libIsolation": true`，并含自 kit #17 起的全部安全/性能/启动修复；每个 hap 的 `libs/arm64-v8a/` 直接携带 254 个 payload 文件与 `.dotnet-payload.json`（原地启动，`dotnet.zip` 为回退）。主选是 kit #28 本身，对照载荷与 P1–P4 探针按交付方指示取用；并列资产 **`aot-haps.tar.gz`**（AOT 试装包）与 **`ohos-interpreter-pack.tar.gz`**（解释器实验）。**注意 `verify-kit.sh` 自 kit #23 起更严（#24 加 payload-in-libs 断言、#25 更新期望值（abc `234620`/`18532`、`dotnet.zip` 254 项、`resources.index` ≤ 2 KiB）、#26 沿用、#27 把 abc 重锚为 `245412`、#28 再重锚 `264136`）**：`FAIL` 会以退出码 1 失败、`WARN` 仍为 `KIT OK`；用 #27 的旧 abc `245412`（或更早的 `234620`）校验本包会 FAIL，属脚本预期；kit #22 全部断言通过，kit #21 及更早的包会被明确报 FAIL（缺 `resources.index` 等真实缺陷，非误报）——旧包请用其自带 verify-kit。
...
- **kit #25 权限链**：该变体的 `module.json` 为上述 5 项权限逐个补齐 `reason`（`$string:permission_reason_*`）
  与 `usedScene`（`abilities=[EntryAbility]`、`when=inuse`）；运行时授权弹窗应显示**理由文案**（见 §8c 判定点），
  系统设置的应用权限页同样显示该理由。打包期另有 feature→permission 矩阵与请求点扫描门禁（strict 可 error）。
...
- **变体摘要**：`module.json` 的 `requestPermissions` 恰为上述 **5 项** ✓；5 个 hap 文件名：
  ...
  以随包 `SHA256SUMS` 为准（解包后 `sha256sum -c SHA256SUMS` 全过即可），kit 整包与内容树见 `device-test-kit` release 说明的「## Integrity」或 `.tar.gz.sha256` sidecar（本轮实测 tar **196,220,486 B** / `091dcc56…`、树 **`0a7a3215…`**）。
...
`[openharmony-host] … bound via alias '…'`、首帧判定、`kit_index_ok` 与 payload-in-libs `payload=yes|no`；`kit_index_ok=no` 请换 kit #22+ 再测），无需手工 grep。JIT/探针判定表与 NativeAOT 指引见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`；kit #25 的判定点（权限弹窗文案/Share 面板/Scan 返回/AOT 启动）见 §8c，kit #26 的判定点（新 payload 首次运行/PLAT-GAP 恢复路径）见 §8d，kit #27 的判定点（无 HMS 降级不抛（Push token / Account 授权 / Map 能力位）/新 payload 首次运行）见 §8e，kit #28 的判定点（Map 覆盖层/LiveView/AOT 启动桥/解释器实验）见 §8f；四轮交接分别为 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md`、`docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`、`docs/plans/2026-09-27-ohos-tester-handoff-kit27.md` 与 `docs/plans/2026-09-26-ohos-tester-handoff-kit28.md`。

### 8c. kit #25 新增判定点（权限弹窗文案 / Share 面板 / Scan 返回 / AOT 启动）

> 本 kit 的 5 个 hap 是 **JIT payload**（hostfxr 回退路径）；Share/Scan 的 sink 在 OpenHarmony SDK
> 下**不注册**（特性探测 `shareDispatch=False`/`scanSupported=False`），面板/扫码 UI 需
> `ARKTS_SDK_FLAVOR=harmony` 的 HarmonyOS SDK 构建 + HMS 设备。没有对应入口的项按
> 「未测（本包无入口）」登记，不要判失败。完整判读见 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md` §2。

| 判定点 | 怎么测 | 期望 | 回传证据 |
|---|---|---|---|
| 权限弹窗文案 | 重签 `hello-maui-app-permissions.hap` → 触发蓝牙/联系人/日历运行时授权（`PRINT` 为 system_grant 不弹） | 弹窗显示**理由文案**（`$string:permission_reason_*` 本地化文本，不是空/通用文案）；允许/拒绝行为同 kit #24 | 弹窗截图 + `files/dotnet-status.txt` 的 `[maui] permission …` 行；`unzip -p <hap> module.json` 的 `requestPermissions` 原文 |
...
> kit #27 继续沿用本节（重建 payload 首次运行、原生桥/回调无 ABI 回归），并把 abc 期望改为 `245412`；kit #28 沿用同
> 口径（abc 期望 `264136`，并新增 §8f 的 Map 覆盖层/LiveView/AOT/解释器判定点）。
...
### 8f. kit #28 判定点（Map 覆盖层 / LiveView / AOT 启动桥 / 解释器实验）

> kit #28（R2）默认 flavor 的 5 个 hap **仍无 Map/LiveView UI 入口**：首要判定仍是**降级不抛**（Map overlay
> 调用、LiveView 三调用）与**重建 payload 首次运行**。真机点亮分别需要 `ARKTS_SDK_FLAVOR=harmony` 构建 +
> AGC 地图 AppKey（Map）、AGC 实况窗权益 + 设备开关（LiveView）；AOT 直启需 `aot-haps.tar.gz` 重签后的 AOT hap；
> 解释器为独立实验资产。没有对应入口/资产时按「未测」登记，**不要判失败**。完整判读见
> `docs/plans/2026-09-26-ohos-tester-handoff-kit28.md` §2。
...
| Map 覆盖层点亮（harmony + AppKey，可选） | harmony flavor 壳 + AGC 地图服务 AppKey → show/hide/区域/标记 | flags bit1=1；地图出现；Ready（+ MarkerClick/CameraIdle）事件可达 | 截图 + flags/事件日志 + AGC 开通截图 |
| LiveView 降级不抛 | 默认 flavor 包上触发 LiveView 探针/自检（TIMER） | `IsSupported=false`；Start/Update/Stop `Unavailable`，**不抛** | 状态原文 + hilog |
| LiveView 点亮（HMS + 权益，可选） | AGC 实况窗权益 + 设备开关开启 → create/update/stop | TIMER 卡片出现/更新/停止；失败码 `1003500004`/`1003500005` 透传 | 卡片截图 + 错误码原文 |
| AOT 启动桥 | `aot-haps.tar.gz` 重签 AOT hap → 安装 → 启动（壳 `start_app` 路由） | 日志 **`aot=1`**、managed 正常输出、**无 `The application to execute does not exist`**；kit 内 JIT hap 仍 `aot=0` 回退且行为同 #27 | 启动日志 + managed 输出 |
| 解释器实验（可选） | 按 §8f 前置替换 payload 两个 `.so`、写 `<files>/interp.txt`（如 `3`）→ 启动 | 日志 **`interp=3 source=file`**；maps 含 `libclrinterpreter.so`、**无匿名 `r-x`**（残余 stub 先记录，勿改 W^X）；managed 正常输出 | `interp=` 行 + maps 摘录 + pack `sha256sum -c` 输出 |
| 新 payload 首次运行（承 #26/#27） | 重签 → 安装默认 hap → 启动 → 跑 5 条冒烟 | 两行 `[maui]` 日志、无崩溃；`verify-kit.sh` 全过（**新 abc 期望 `264136`/`18532`**；`dotnet.zip` 254、index ≤ 2 KiB） | 启动两行日志 + `files/dotnet-status.txt`；失败附 `tester-run.sh` 证据包 |
