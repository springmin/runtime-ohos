...
校验方式：解包后运行 `sha256sum -c SHA256SUMS`（逐文件校验）；`SHA256SUMS` 由交付方在打包时生成并随包分发。kit 整包与解压内容树的数字见 `device-test-kit` release 说明的「## Integrity」小节（`workload-latest` 镜像同值）或 `.tar.gz.sha256` sidecar。当前发布为 **kit #23**（2026-09-24，工具刷新：强化后的 `verify-kit.sh`（逐 hap 深度断言）与 `tester-run.sh` v7 入包；hap 负载与 #22 相同）：包内 5 个 hap 均带 `"libIsolation": true`，并含自 kit #17 起的全部安全/性能/启动修复；主选是 kit #23 本身，对照载荷与 P1–P4 探针按交付方指示取用。**注意 `verify-kit.sh` 自 kit #23 起更严**：`FAIL` 会以退出码 1 失败、`WARN` 仍为 `KIT OK`；kit #22 全部断言通过，kit #21 及更早的包会被明确报 FAIL（缺 `resources.index` 等真实缺陷，非误报）——旧包请用其自带 verify-kit。
...
`hilog/hilog-applib.txt`、`hilog/hilog-dlopen.txt`、`hilog/hilog-bootstrap.txt`、`device/app-libs-arm64.txt`、
`device/payload-files.txt`/`payload-marker.txt` 与 `meta/kit-selfcheck.txt`（含别名注册行
`[openharmony-host] … bound via alias '…'`、首帧判定与 `kit_index_ok`；`kit_index_ok=no` 请换 kit #22+ 再测），无需手工 grep。
