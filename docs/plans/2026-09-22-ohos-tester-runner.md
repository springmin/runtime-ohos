...
> 当前发布相关：**kit #25**（2026-09-25 发布；权限链 `reason`/`usedScene` + Share/Scan 特性探测降级 + AOT 启动路径，并在 #24 的 payload-in-libs 基础上：hap 内 `libs/arm64-v8a/` 原地携带 254 payload + `.dotnet-payload.json`，`dotnet.zip` 回退；整包 tar.gz 数字入口见 release「## Integrity」；kit 内强化 verify-kit 自检应报 0 FAIL / 0 WARN）。2026-09-24 真机里程碑（kit #18 + 测试方 5 项本地修复首次完整运行）见 `docs/plans/2026-09-24-ohos-device-milestone.md`；**stock kit（#22 起，含 #25）尚未上机**，本轮归档即首次复测证据。JIT A/B 与判定表见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`，kit #25 判定点（权限弹窗文案/Share 面板/Scan 返回/AOT 启动）见 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md`。
...
- **exec-memory 证据**（v8；kit #24 起，kit #25 沿用）→ `hilog/hilog-execmem.txt`：把同一批 hilog 窗口按 `OHOS_DOTNET probe:|xwe=` 过滤，保留 `OHOS_DOTNET probe: 1=… 2=… 3=… 4=…` 与 `[openharmony-host] … xwe=0|1 source=…` 行；`summary.txt` 记 `execmem_capture`（ok/not_captured）、`execmem_lines`（0 = 未捕获，加长 `--capture` 重跑）。判定表见 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md` §3（指向 kit #24 交接 §5）。
...
| `kit_index_ok=no` | 当前 kit 至少一个 hap 缺 `resources.index`：换 **kit #22+（当前 #25）** 再测（这类包会在托管 bootstrap 前失败）。`<unavailable>` = 本机缺 `python3`/`unzip` 或 kit 无 hap，自检未完成，不影响安装流程 |
