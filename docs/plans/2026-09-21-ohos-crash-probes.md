...
the current kit (#23, same payload as #22) and are **kept as harmless hardening**: the 2026-09-24 device run showed the
black-screen chain was the host dlopen + bootstrap issues, not this name/path half (see
`2026-09-24-ohos-device-milestone.md` §2). The diagnostic is part of `tester-run.sh` v7 (`hilog-applib` /
...
0b. **当前 kit（#23，同 #22 负载）**已含上述全部修复与启动修复（P17 解压跳过、H7 rawfile fd 直读、headless abc `13.0.1.0`），并含设备里程碑回灌（宿主 `DT_NEEDED` 5 + 可选 API 按需 dlsym、HAP `resources.index`、ZIP offset/length + mkdir、DevEco 工程布局）；`tester-run.sh` v7 会自动采集 app-lib 证据与别名注册行（`hilog-applib`/`hilog-dlopen`/`app-libs-arm64`），并新增 `hilog-bootstrap`（bootstrap/rawfile 失败特征）与 `payload-files`/`payload-marker`、`meta/kit-selfcheck`（`kit_index_ok`），首帧判定一并写进 `summary.txt`。数字入口 = release「## Integrity」；里程碑见 `2026-09-24-ohos-device-milestone.md`。
