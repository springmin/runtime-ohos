...
> 当前发布 = **kit #28**（2026-09-26，R2）：**Map 覆盖层**（`ARKTS_SDK_FLAVOR=harmony` 壳 + AGC 地图 AppKey；默认 flavor 下 `IsSupported` 可为 true 而 `IsOverlayAvailable=false`，show/hide/区域/标记等调用**降级不抛**）、**LiveView 特性探测**（无 Kit/权益时 `IsSupported=false`、Start/Update/Stop `Unavailable` 不抛，TIMER 模板）、**壳 `start_app` AOT 启动桥**（探测 `lib<stem>.so` → `openharmony_app_main`，日志 `aot=1`，失败 `aot=0` 回退 JIT）与**解释器实验资产**（`<files>/interp.txt` → `DOTNET_InterpMode`，日志 `interp=3 source=file`）。宿主导出契约 **134/134**；ui/shell abc → **264,136 B**（headless 18,532 B）；交互门禁 **334/floor 314**。载荷/管道沿用 #24–#27（`libs/arm64-v8a/` 原地携带 254 个 payload 与 `.dotnet-payload.json`；`dotnet.zip` 254 项仅作回退）；KIT-EXT2（Push/Account/Map 探测）、权限链、Share/Scan 探测降级与 P2-INTEROP/TASK-MIG/PLAT-GAP 均不变。主选就是 kit #28 本身（含 kit #22 起的设备回灌与自 #17 起全部安全/性能/启动修复）。对照载荷（dynpkg/normalized/importb/importd/importprobe a–c）与 P1–P4 探针仍在同一 release，按交付方指示取用；**并列资产** = `aot-haps.tar.gz`（AOT 试装包，17,090,044 B / `67519d11…`）与 `ohos-interpreter-pack.tar.gz`（解释器实验，2,419,988 B / `a10699b3…`）。本轮判定点（**Map 覆盖层 show/hide/区域/标记/Ready/MarkerClick/CameraIdle、LiveView create/update/stop、AOT `aot=1`、interpreter `interp=3 source=file` + maps 无匿名 `r-x`、无 HMS 降级不抛、新 payload 首次运行**）见 `docs/plans/2026-09-26-ohos-tester-handoff-kit28.md`；整包数字入口 = release「## Integrity」（本轮实测：tar **196,220,486 B** / `091dcc56…`、树 **`0a7a3215…`**、sidecar **`d7efd251…`**、5 hap ~75.67 MB、bundle **30,525,614 B** / `7d614517…`）。
> **设备里程碑（2026-09-24）**：kit #18 + 测试方 5 项本地修复后首次完整运行成功（`managed app hello-maui-app.dll started (UI shell)`、进程存活、无崩溃）；**stock kit（#22 起，含本轮 #28）尚未上机**，本轮就是它的首次设备复测。详见 `docs/plans/2026-09-24-ohos-device-milestone.md`。
...
> **verify-kit 变严了（kit #23 起；旧包会 FAIL 属预期）**：kit #23 包内 `verify-kit.sh` 在逐文件校验之外，逐 hap 断言 `resources.index`（缺/空 = FAIL）、abc 版本 `13.0.1.0` 与当前壳大小、`libs/arm64-v8a` 14 个 `.so`、`dotnet.zip` 不含 `.so`、宿主 ELF `DT_NEEDED`/未定义符号白名单。FAIL → 退出码 1；WARN → 打印但仍是 `KIT OK`。**kit #24 起再加 payload-in-libs 断言**：每个 hap 的 `libs/arm64-v8a/.dotnet-payload.json` 必须存在且自洽（入口程序集已入 libs、条目数与实际文件数一致、`payloadEntries == zipEntries`、`zipSha256` 匹配打包字节），缺失/不一致 = FAIL（该 hap 会回退到被设备拒绝的 data 目录解包）。**kit #25 的期望值更新为**：abc `234620`/`18532`（旧壳大小仍按 WARN 提示）、`dotnet.zip` 254 项、`resources.index` 上限 2 KiB（1588/1780）；**kit #26 期望值不变**（hap 内 hosting DLL 变大属预期，不影响这些断言）；**kit #27 把 abc 期望重锚为 `245412`**（headless `18532` 与 `dotnet.zip`/index 不变；`ohos-workload 898f4a1`）、**kit #28 再重锚 `264136`**（`ohos-workload 112b6e9`）—— 用 #27 的旧值 `245412`（或更早的 `234620`）校验本包会 FAIL，属脚本预期。**kit #22 全部通过；kit #21 及更早的包会被明确报 FAIL（缺 index、旧宿主 NEEDED 等真实缺陷），不是误报** —— 旧包请用它自带的 verify-kit，强化结果请用 kit #22+（当前 #28）。新增可选参数 `--expected-abc` / `--host-deps`，`--anchor`/`--tree-digest` 用法不变。
...
**哈希一律以发布说明为准（重签/重打包会变），本文只记本轮实测短哈希**：整包 sha256 与解压内容树 sha256 见 `device-test-kit` release 说明的「## Integrity」小节（`workload-latest` 镜像同值），③ 的参数就用那里的 tree sha256；`tester-run.sh`（v8）会把整包摘要与主 hap 摘要写进证据包 `meta/kit-hap-sha256.txt` 与 `summary.txt` 的 `main_hap_sha256`，可直接对照。当前发布为 **kit #28**（2026-09-26；R2 Map 覆盖层/LiveView 探测/`start_app` AOT 桥/解释器实验；tar **196,220,486 B** / `091dcc56…`、树 **`0a7a3215…`**、sidecar `d7efd251…`、5 hap ~75.67 MB；`tester-run.sh` 仍为 v8），包内版本原文见 `最终状态.md`「发布物」/`README-交付说明.md`「构建基线」。重签、预签或重新打包后的哈希必然不同 —— 以发布说明与随包 `SHA256SUMS` 为准。
...
- **kit #26（P2-INTEROP + TASK-MIG + PLAT-GAP）**：hosting 桥全量源生成 **`LibraryImport`**（125 处 = 44 hosting + 81 MAUI 切片；`DllImport` 归零；导出契约 **118/118** 不变，`StringMarshalling.Utf8`、thunk 用 `[UnmanagedFunctionPointer(Cdecl)]`）→ hap 内 hosting DLL 47,616 → **55,808 B**；hap 打包任务改为 **pack 内编译程序集**（`tools/Microsoft.OpenHarmony.Tasks.dll` 32,256 B，共 6 个任务程序集；`OpenHarmony.Hap.targets` 拆分 + 新增 `PlatformItems.targets`）；**PLAT-GAP 消解**：AspNetCore KFR 固定到发布波段 `11.0.0-rc.1.26425.128`、默认 `RuntimeIdentifier=openharmony-arm64`、`EnableAppHostPackDownload=false` —— 消费方（如 BlazorWebView 项目）**不再需要**工程级 KFR/RID 规避。指纹：hap ~75.47 MB（zip 278；`libs` 269 = 14 `.so` + 254 payload + marker）、abc 234,620/18,532（不变）、宿主 240,544（hap 内）/236,448（pack，均不变）、index 1588/1780、`dotnet.zip` 254/0 `.so`；整包 tar 195,748,984 B（哈希见 release「## Integrity」）。判定点见 `docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`。
- **kit #27（KIT-EXT2：Push/Account/Map 探测 + 12 kit sink/130 导出 + marshal-off）**：壳对三个 HMS Kit 做特性探测（`@kit.PushKit` 的 `getToken()`/`deleteToken()`、`@kit.AccountKit` 的 `createAuthorizationWithHuaweiIDRequest()` + `getQuickLoginAnonymousPhone`、`@kit.MapKit` capability bits），**变量 import 成功才注册 sink；无 Kit/AGC/HMS 一律 `Unavailable`/null/false 且不抛**；宿主新增 `ohos_host_push_*`/`ohos_host_account_*`/`ohos_host_map_*`（含错误码映射 `1000900010`/`1000900012`、`1001502014`/`1001500001`），导出契约 **118/118 → 130/130**，**FIX-R1-NAPI-6D** 边界加固；反向条目（host→托管回调）改走 **off-runtime marshaller**（`[UnmanagedCallersOnly]` + `delegate* unmanaged[Cdecl]` thunk）；交互门禁 **329/floor 309**；`OpenHarmonyHapPayloadZip` opt-out 三套 preview pack 同步。指纹：hap 各 **~75.56 MB**（zip 278；`libs` 269 = 14 `.so` + 254 payload + marker）、abc **245,412**/`18532`（ui/shell 重建；headless 不变）、宿主 **265,120**（hap 内）/`261,024`（pack）、hap 内 hosting DLL 55,296 B、index 1588/1780、`dotnet.zip` 254/0 `.so`；整包 tar 195,951,029 B（哈希见 release「## Integrity」）；包内 `verify-kit.sh` 的 abc 期望已重锚 245,412。判定点见 `docs/plans/2026-09-27-ohos-tester-handoff-kit27.md`。
- **kit #28（R2：Map 覆盖层 + LiveView 探测 + `start_app` AOT 桥 + 解释器实验）**：**Map 覆盖层（方案 a）** 位于 harmony-flavor 专属模块（字面量 `@kit.MapKit` + `NodeController`/`BuilderNode`），托管 `OpenHarmonyMap` 提供 show/hide/close/区域/标记与 `Ready`/`MarkerClick`/`CameraIdle`；**默认 flavor 下 `IsOverlayAvailable=false`、overlay 调用降级不抛**（真机点亮需 `ARKTS_SDK_FLAVOR=harmony` + AGC 地图 AppKey）。**LiveView** 为 `canIUse`+`@kit.LiveViewKit` 双门探测（TIMER 场景 create/update/stop），无 Kit/权益时 `IsSupported=false`、Start/Update/Stop `Unavailable` 不抛；宿主新增 `ohos_host_liveview_*`，导出契约 **130/130 → 134/134**。**壳 `start_app` AOT 桥**：探测 `lib<stem>.so` → `openharmony_app_main`，命中直启（日志 `aot=1`），失败 `aot=0` 回退 JIT。**解释器实验**：`ohos-interpreter-pack.tar.gz` + `<files>/interp.txt` → `DOTNET_InterpMode`（日志 `interp=3 source=file`）。ui/shell abc **264,136 B**（headless 18,532）、交互门禁 **334/floor 314**；并列资产 `aot-haps.tar.gz`（17,090,044 B / `67519d11…`）与解释器 pack（2,419,988 B / `a10699b3…`）；整包 tar **196,220,486 B** / `091dcc56…`（树 `0a7a3215…`、sidecar `d7efd251…`、5 hap ~75.67 MB；哈希以 release「## Integrity」为准）；包内 `verify-kit.sh` 的 abc 期望已重锚 `264136`。判定点见 `docs/plans/2026-09-26-ohos-tester-handoff-kit28.md`。
- 主选仍是 **kit #28 本身**（含 `libIsolation` 与全部修复；R2 变化在 Map 覆盖层（harmony 专属）/LiveView 探测/AOT 壳桥/解释器开关，均不新增 UI 入口）；对照载荷与探针只在交付方指定时使用。
...
- **kit #28 交接（R2 增量：Map 覆盖层 / LiveView / AOT 启动桥 / 解释器实验 + 判定点）：`docs/plans/2026-09-26-ohos-tester-handoff-kit28.md`**
- kit #27 交接（KIT-EXT2 增量 + 无 HMS 降级不抛（Push token / Account 授权 / Map 能力位）/ 新 payload 首次运行 / 权限弹窗 / Share / Scan / AOT 判定点；历史）：`docs/plans/2026-09-27-ohos-tester-handoff-kit27.md`
- kit #26 交接（P2-INTEROP/TASK-MIG/PLAT-GAP 增量 + 新 payload 首次运行判定点；历史）：`docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`
