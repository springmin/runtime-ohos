...
- **kit #28（R2：Map 覆盖层 + LiveView 探测 + `start_app` AOT 桥 + 解释器实验）**：**Map 覆盖层（方案 a）** 位于 harmony-flavor 专属模块（字面量 `@kit.MapKit` + `NodeController`/`BuilderNode`），托管 `OpenHarmonyMap` 提供 show/hide/close/区域/标记与 `Ready`/`MarkerClick`/`CameraIdle`；**默认 flavor 下 `IsOverlayAvailable=false`、overlay 调用降级不抛**（真机点亮需 `ARKTS_SDK_FLAVOR=harmony` + AGC 地图 AppKey）。**LiveView** 为 `canIUse`+`@kit.LiveViewKit` 双门探测（TIMER 场景 create/update/stop），无 Kit/权益时 `IsSupported=false`、Start/Update/Stop `Unavailable` 不抛；宿主新增 `ohos_host_liveview_*`，导出契约 **130/130 → 134/134**。**壳 `start_app` AOT 桥**：探测 `lib<stem>.so` → `openharmony_app_main`，命中直启（日志 `aot=1`），失败 `aot=0` 回退 JIT。**解释器实验**：`ohos-interpreter-pack.tar.gz` + `<files>/interp.txt` → `DOTNET_InterpMode`（日志 `interp=3 source=file`）。ui/shell abc **264,136 B**（headless 18,532）、交互门禁 **334/floor 314**；并列资产 `aot-haps.tar.gz`（17,090,044 B / `67519d11…`）与解释器 pack（2,419,988 B / `a10699b3…`）；整包数字入口见 release「## Integrity」；包内 `verify-kit.sh` 的 abc 期望已重锚 `264136`。判定点见 `docs/plans/2026-09-28-ohos-tester-handoff-kit28.md`。
...
- **kit #28 交接（R2 增量：Map 覆盖层 / LiveView / AOT 启动桥 / 解释器实验 + 判定点）：`docs/plans/2026-09-28-ohos-tester-handoff-kit28.md`**
