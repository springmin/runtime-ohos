...
> 当前发布 = **kit #26**（2026-09-26，P2-INTEROP + TASK-MIG + PLAT-GAP）：托管 hosting 桥全量切到源生成 **LibraryImport**（125 处 = 44 hosting + 81 MAUI 切片；`DllImport` 归零，导出契约仍 **118/118**，hap 内 hosting DLL 55,808 B）；hap 打包任务改为 pack 内编译程序集（`tools/Microsoft.OpenHarmony.Tasks.dll`，共 6 个任务程序集；`OpenHarmony.Hap.targets` 拆分 + 新增 `PlatformItems.targets`）；**PLAT-GAP 消解**（AspNetCore KFR 固定到 `11.0.0-rc.1.26425.128` 波段、默认 `RuntimeIdentifier=openharmony-arm64`、`EnableAppHostPackDownload=false`）—— 消费方**不再需要**逐项目 KFR/RID 规避。载荷/管道沿用 #24/#25：`libs/arm64-v8a/` 原地携带 254 个 payload 与 `.dotnet-payload.json`（签名 hap ~75.47 MB；`dotnet.zip` 254 项仅作回退）；权限链、Share/Scan 探测降级与 AOT 启动路径不变。主选就是 kit #26 本身（含 kit #22 起的设备回灌、#24 的 payload-in-libs/显式 W^X=0、#25 的权限/探测/AOT 与自 #17 起全部安全/性能/启动修复）。对照载荷（dynpkg/normalized/importb/importd/importprobe a–c）与 P1–P4 探针仍在同一 release，按交付方指示取用；本轮判定点（权限弹窗 / Share 面板 / Scan 返回 / AOT 启动 / **新 payload 首次运行**）见 `docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`。
> **设备里程碑（2026-09-24）**：kit #18 + 测试方 5 项本地修复后首次完整运行成功（`managed app hello-maui-app.dll started (UI shell)`、进程存活、无崩溃）；**stock kit（#22 起，含本轮 #26）尚未上机**，本轮就是它的首次设备复测。详见 `docs/plans/2026-09-24-ohos-device-milestone.md`。
...
- 主选仍是 **kit #26 本身**（含 `libIsolation` 与全部修复；#26 相对 #25 未见新 UI 入口，变化在互操作层/打包层/恢复路径）；对照载荷与探针只在交付方指定时使用。
...
- **kit #26 交接（P2-INTEROP/TASK-MIG/PLAT-GAP 增量 + 权限弹窗 / Share 面板 / Scan 返回 / AOT 启动 / 新 payload 首次运行判定点）：`docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`**
