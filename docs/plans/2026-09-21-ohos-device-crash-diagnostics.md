...
>
> **2026-09-27 更新（kit #27，当前）**：KIT-EXT2 —— 壳对 Push/Account/Map 三个 HMS Kit 做特性探测（无 Kit/AGC/HMS 时 sink 不注册、值 `Unavailable`/null/false，**不抛**）、宿主新增 12 个 kit sink（导出契约 118/118 → **130/130**）+ FIX-R1-NAPI-6D 边界加固、反向条目改走 off-runtime marshaller（`[UnmanagedCallersOnly]` + `delegate* unmanaged[Cdecl]` thunk）；ui/shell abc → **245,412 B**、hap 内宿主 → **265,120 B**、hosting DLL → 55,296 B。**启动崩溃判定（`probe:`/`xwe`、P1–P4、bootstrap）不变**，kit #27 回归点 = 重建 payload（新 abc + 新宿主 + marshal-off 托管）仍正常启动、原生桥/回调行为与 #26 一致、无 HMS 探测不崩。`verify-kit.sh` 的 abc 期望已重锚 `245412`（用 `234620` 旧值校验会 FAIL，属预期）。本轮增量判定点见 `docs/plans/2026-09-27-ohos-tester-handoff-kit27.md` §2。
>
...
7. [ ] **JIT 相关（kit #24 起，kit #27 沿用）**：`hilog-execmem.txt` 原文（`probe:` 行 + `xwe=` 行）与 `summary.txt` 的 `execmem_lines`；做过 A/B 的附两轮对照。判定表见 `docs/plans/2026-09-27-ohos-tester-handoff-kit27.md` §4 / `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md` §5。
