...
> 当前发布 = **kit #24**（2026-09-24，payload-in-libs + 显式 W^X=0 + exec-memory 探针：hap `libs/arm64-v8a/` 原地携带 payload + `.dotnet-payload.json`（`dotnet.zip` 回退）、宿主两条启动路径显式 `DOTNET_EnableWriteXorExecute=0`、`xwe.txt` A/B 与 `OHOS_DOTNET probe:` 探针；含 kit #22 的设备回灌与自 #17 起全部修复）；里程碑与复测判定点见 `docs/plans/2026-09-24-ohos-device-milestone.md`；JIT/NativeAOT 交接见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`。包内
...
- 一页回传模板：`docs/plans/2026-09-21-ohos-device-report-template.md`；若启动即崩，`ReferenceError: Cannot find module 'ets/entryability/EntryAbility' , which is application Entry Point`（约 1 秒退出 / `exit 254`）是旧 kit（kit #10 之前）的壳 abc 入口 record 缺陷，**kit #24 已不含**（修复历史见 `docs/plans/2026-09-22-ohos-startup-crash-rootcause.md`；2026-09-24 设备证据修正见其 §5f）；其他崩溃附 `docs/plans/2026-09-21-ohos-crash-probes.md` 的 P1–P4 探针结果（含免安装 14 库自检）与 `tester-run.sh` v8 的 app-lib/dlopen/bootstrap/payload/execmem 证据；里程碑背景见 `docs/plans/2026-09-24-ohos-device-milestone.md`，JIT 判定与 NativeAOT 指引见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`。
