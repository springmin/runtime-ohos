...
On CI: the only non-Helix red check on this head is `runtime (Build osx-arm64 Debug Libraries_CheckedCoreCLR)` in build 1605552 — `sccache: Compiler killed by signal 11` while compiling `src/coreclr/jit/utils.cpp` -> `utils.cpp.o`. That is a compiler-process crash in a file untouched by this PR and looks like an infrastructure flake, so a rerun should clear it.

Requested actions:
1. Please rerun the failed osx-arm64 leg (or close/reopen) so the check goes green.
