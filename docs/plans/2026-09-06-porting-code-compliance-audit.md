...
- Resolved 2026-09-16: the obsolete `libnuma-shim.so` + sources were removed (the runtime embeds the fix) and the two committed nupkgs (reference runtime pack, stock crossgen2) are downloaded on demand with pinned sha256 checks; evidence `final-evidence/artifact-hygiene-20260916.txt`.
