...
- **日期**：2026-09-25 · **范围**：`ohos-workload`（壳/宿主/打包）、`maui-ohos`（OpenHarmony 平台切片）、`sdk-ohos`（SDK 打包/codesign）、`runtime-ohos`（文档/契约）。
...
| 互操作/AOT | 6 | 2 | 4 | 2（`check-host-exports.py` 静态 + `host-export-contract.yml`） | 5 | 1 |
| MSBuild | 10 | 3 | 7 | 3（`lint-packs` / 双导入 fixture / `ridgraph-sync.yml`） | 8 | 2 |
| **合计** | **30** | **13** | **17** | **10** | **25** | **5（+2 SDK 基建）** |
...
| HMS Kit（Share/Scan/Map… 在 OpenHarmony SDK 下静态不可编译） | `hmos-one-sdk-skill` / KIT-GAP：Kit 需特性探测 + 优雅降级 | 偏差·中 | 仅文档门控 | Share/Scan `canIUse` + 变量 import 探测、缺失降级；`ARKTS_SDK_FLAVOR=harmony` 脚手架 | 探测已落地 `652c356`/`68220cd`；真机验收排期（§3.2-③） |
...
门禁/数字：`scripts/build-host.sh` 3 次全绿（NEEDED 5/5、UND denylist 0、exports 114/114 → 后并 118/118、selfsign、无 libhostfxr）；交互套件 **320/320 floor=300**（修复时点，`fix-napi/verify4.txt`）、A7 pin 7/7、像素 `PIXEL ASSERTIONS PASSED`；pin 更新 `c3fa422`/`6a88b83`。
...
| `contract-check.txt:1`（121 处 `[DllImport]` 声明） | `dotnet-pinvoke`：优先 `[LibraryImport]` 源生成 + 回调 | 偏差·P2 | — | 分批迁移 `[LibraryImport]`/回调并加分析器门禁 | **排期**（§3.2-②） |
...
| `OpenHarmony.Hap.targets:105,161,256,326,403,632`（6 个内联 `RoslynCodeTaskFactory` 任务） | `msbuild-antipatterns`：内联任务不可测/不可复用 | 偏差·P2 | — | 迁 `Microsoft.OpenHarmony.Tasks.dll`（net11.0 + 单测 + 三 pack `tools/`） | **排期** RELEASE-25（§3.2-①） |
| 打包 clean 无功能回归；`ridgraph-sync.yml` pin 需锁步 bump | `incremental-build` | 偏差·低 | — | 真实 publish `dotnet clean` 回归 + pin 联动 | **排期** RELEASE-25（§3.2-④） |
...
门禁/数字：`lint-packs` + `selftest-packs` 14/0（双导入 fixture）、`selftest-hap-targets` 30/0、sdk codesign FileWrites 5/5、`preflight` 全绿（RID/lint/paths + 29 脚本 + markdownlint 0）；ohos-workload master 的 5 条 workflow（interaction/pixel/host-export-contract/ridgraph-sync/markdownlint）全部 success。
...
- **互操作/AOT**：5 个 C 链接符号 + 118 导出契约 + `nm`/CI 门禁（`88ee1d7`→`ccf61e6`）；STJ 源生成 + trim/AOT 分析器门禁、IL 告警 0（`635bbd0e`/`6062d796`/`5cd7688d`）；AOT 单入口（`f3514a6`/`2e98249`）；Share/Scan EntryPoint 并入契约（`fc7fbfdc`）。
- **MSBuild**：module.json/工具链/Blazor/AfterPublish/FileWrites（`023d1d7`）；悬空 props/targets + 双导入守卫（`a285f7b`）；RID 图单源（`37eed53`）；文档契约（`e9cd1f4`）；相对路径门禁（`88ca8eb`/`fd913a2`）；sdk codesign FileWrites（`5fc073ed8f`）。

### 3.2 排期项（含阻塞点）

1. **内联 MSBuild 任务迁移**（M8）：需新建 net11.0 任务项目、DLL 提交三 pack `tools/`、全套 hap publish 复验；随 RELEASE-25。
2. **P2 · 121 处 `[DllImport]` → `[LibraryImport]`/回调**（I5）：阻塞 = 切片需上游 MAUI 同步 + 本机 SDK 无 ilc/runtime pack（`dotnet publish -r openharmony-arm64 -p:PublishAot=true` 报 NETSDK1083/1203，仅供 host AOT 路线）。
3. **NAPI 6d 剩余项**（drawing RAII / effect 所有权 / location-IME 生命周期 / NodeContent 身份 / 原子发布注册表 / 缓冲按需）：阻塞 = `comp-napi/report.md` 原文缺失，需重做只读定位；互操作侧已收口，无并发写冲突。
4. **打包 clean 功能回归 + RID 图 pin 联动**（M9）：需真实 publish（设备 kit 或 stub 工具链 fixture）与 SDK 重建；`ridgraph-sync.yml` 现 pin `32e1719359`，规范图变更须锁步 bump。
5. **sdk 测试接入 CI**：`eng/ohos-install/tests/` 3 个脚本（filewrites 5/5、hostfeed、installer 43/43）尚未挂 workflow；阻塞 = runner 需 NDK/离线资产或改为纯静态门禁。
6. **sdk `ohos-full-build` runner 红**：run `36131775031`（illink `MSB6006` exit 150）与 `36129693381`（host subset 无 corehost apphost）；阻塞 = 需 runner 侧复现/缓存修复后复跑，本地无同构环境。
7. **HMS 真机验收**（A5）：Share `shareCompleted`、Scan `originalValue`、HarmonyOS flavor abc 装载；见 §4。

## 4. 仍需外部条件与文档回填

- **HMS 设备**：Share/Scan 探测与降级链路已落地，真机验收需 HMS 设备；Push/Account/Map/LiveView/Payment 另需 AGC 开通/审批（Push `1000900010`、Account `1001502014`、Map AppKey、实况窗权益、支付商户）。
- **HarmonyOS SDK**：`ARKTS_SDK_FLAVOR=harmony` 完整构建需装有 DevEco/HMS SDK 的机器（本机仅 OpenHarmony SDK 26.0.0.18）。
- **真机项**：NAPI TSFN 同步应答阻塞/超时、env cleanup 页销毁顺序、AOT 单入口冒烟、SecureStorage HUKS 分支。
- **文档回填**：KIT-GAP 矩阵的 KIT-IMPL 回填已入库 `8b37c37`；`2026-09-22-ohos-maui-coverage-matrix.md` 仍把「Share Kit 多文件」列为 SDK 阻塞、数字仍为 315/floor 295 —— 应补 Share/Scan 探测落地与 326/floor 306（另一代理在改 release-manifest，本报告未触碰）。

## 5. 与既有合规线交叉引用

- **R1–R11**（`2026-09-23-ohos-pr-review-compliance.md`）：本轮改动落在壳/宿主/打包，不改平台标识、RID 图（R2/R4 保持单一来源与字节一致）、`eng/common`（R6）与 `DOTNET_` 口径；R1–R11 硬违例仍为 0。
...
3. NAPI 6d 未做（缺审计原文定位）；本次未包含其验收。
4. 真机与 HMS/AOT 项（NAPI 同步应答、env cleanup 顺序、Share/Scan、HarmonyOS flavor、AOT 冒烟）均未上机。
5. sdk `ohos-full-build` 两次失败的根因未在本地复现，仅按日志归因。
6. 文中数字为快照：交互 326/floor 306 与像素取自 2026-09-25 20:00 本地 `preflight`；118/118 取自 18:08 `build-host`；CI 5/5 为 master tip `2c1a2cb` 的 run 记录。
