...
| Workload version | `1.0.0-preview.23` (each pack can have its own version) |
...
```
openharmony-workload-1.0.0-preview.23/
...
- Pack ids/versions come from `WorkloadManifest.json`; the feed files are
  ...
  (e.g. `Microsoft.OpenHarmony.Sdk.1.0.0-preview.23.nupkg` next to the BCL runtime pack
...
| No debug channel | ask the device admin/IT, or have the tester self-sign | see the signing guide |
...
sh scripts/sign-for-device.sh <UDID> --out /tmp/app-<name>.hap --version 1.0.0-preview.23
```
...
changes the hash). Alternative self-service paths (DevEco auto-signing, release-type
self-signing for OpenHarmony-only devices) are documented in the signing guide
(`docs/plans/2026-09-19-ohos-signing-and-udid-guide.md`).
...
| `2026-09-19-ohos-signing-and-udid-guide.md` | 9568344 / UDID / re-signing / self-signing (zh-CN) |
...
| `2026-09-19-ohos-code-audit.md` | five-repo audit: fixes, measurements, per-TFM API band, BlazorWebView readiness (§19, §24, §25) |
| `2026-09-18-ohos-device-validation-checklist.md` | step-by-step device validation with artifacts/commands/expected results |
