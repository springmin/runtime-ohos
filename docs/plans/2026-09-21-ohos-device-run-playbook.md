...
> 当前发布 = **kit #25**（2026-09-25，权限链 + Share/Scan 探测降级 + AOT 启动路径：权限变体 5 项权限带 `reason`/`usedScene`（弹窗显示理由文案）、Share/Scan 在 OpenHarmony SDK 上 `shareDispatch=False`/`scanSupported=False` 干净降级（`ARKTS_SDK_FLAVOR=harmony` 才启用面板/扫码）、宿主新增 AOT 启动路径（`lib<stem>.so` → `openharmony_app_main`，hostfxr 回退）；并在 #24 的 payload-in-libs（`libs/arm64-v8a/` 原地携带 254 payload + `.dotnet-payload.json`，签名 hap ~75.43 MB）/显式 W^X=0/exec-memory 探针之上；含 kit #22 的设备回灌与自 #17 起全部安全/性能/启动修复）。数字入口 = release「## Integrity」；里程碑与复测判定点见 `docs/plans/2026-09-24-ohos-device-milestone.md`；本轮判定点见 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md`，JIT/NativeAOT 说明见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`。
...
或直接点桌面图标。首帧应为**黑色导航栏 + 标题「Root」**的长列表；若黑屏/闪退，先记录并走第 7 节回传。2026-09-24 里程碑：kit #18 + 测试方 5 项本地修复已跑到 `managed app hello-maui-app.dll started (UI shell)`、进程存活、无崩溃；**stock kit（#22 起，含 #25）的首次设备复测就是本轮**（失败分支判定见 `docs/plans/2026-09-24-ohos-device-milestone.md` §6）。若崩在 CoreCLR 初始化/JIT（`SEGV_ACCERR`）：kit #24/#25 不含 seccomp 拦截器，先确认没有叠加旧本地补丁，再按 `docs/plans/2026-09-25-ohos-tester-handoff-kit25.md` §3 / `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md` §5 读 `probe:` 行并做 `xwe.txt` A/B。kit #25 另有 AOT 启动路径（app export + hostfxr 回退）：本包的 JIT hap 应仍走回退启动（即最直接的回归检查）。
...
5. 整轮报告推荐用 `tester-run.sh`（v8）生成 `tester-report-<时间戳>.tar.gz`（连同 `.sha256`）：自动收录 hilog（含 `hilog-applib.txt`/`hilog-dlopen.txt`/**`hilog-bootstrap.txt`**/**`hilog-execmem.txt`**）、kmsg、`device/app-libs-arm64.txt`、`device/payload-files.txt`/`device/payload-marker.txt`、`meta/kit-hap-sha256.txt` 与 `meta/kit-selfcheck.txt`，字段与旧版兼容；若 `summary.txt` 报 `kit_index_ok=no` 请换 kit #22+ 再测，`bootstrap_errors`/`rawfile_errors>0` 时附 `hilog-bootstrap.txt`（不影响退出码）。**JIT 判定（kit #24 起，kit #25 沿用）**：附 `hilog-execmem.txt`（`probe:` 行 + `xwe=` 行）与 `summary.txt` 的 `execmem_lines`；判定表与 A/B 见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md` §5（kit #25 索引见交接 §3）。注意 kit #24 起 `payload_present=no` 属正常（payload 在 hap `libs/` 原地运行）。kit #25 的权限变体另附弹窗截图 + `module.json` 的 `requestPermissions` 原文（理由文案判定点）。
