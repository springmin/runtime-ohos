...
> 评审整改（REV/RA/RB/RC-A..D）、真实缺陷修复（PI1）与 UX 深化（PJ1/PJ2）随 **kit #14** 出包；**当前发布 = kit #26**（2026-09-26，P2-INTEROP 全量 `LibraryImport` hosting（125 处，`DllImport` 归零，导出契约 118/118）+ TASK-MIG 打包任务程序集化（pack `tools/Microsoft.OpenHarmony.Tasks.dll`）+ PLAT-GAP 消费方缺省化（AspNetCore KFR 固定 `11.0.0-rc.1.26425.128`、默认 `RuntimeIdentifier=openharmony-arm64`、`EnableAppHostPackDownload=false`）；权限链/Share-Scan 探测降级/AOT 启动路径沿用 #25，并在 #24 的 payload-in-libs/显式 W^X=0/exec-memory 探针之上；`tester-run.sh` 仍为 v8；见 §11 与
> `2026-09-24-ohos-device-milestone.md`）、离设备套件 **315 条 / floor 295**（preflight 与 CI 同源），见 §4、§5、§10、§11）。
...
- **交互套件（当前）**：`test/maui-platform-verify` 现为 **315 条** `[verify]`、门限 **floor 295**（套件自报 `[suite]` 行，
  preflight 与 CI 同源解析同一行，不再有 226/288 双字面量；`ohos-workload 92f7555`/`a10b73e` 与 `d336bba`+`57d18c0`）。
...
## 5. 发布物（当前 = `device-test-kit` release 说明的「## Integrity」小节，当前编号 kit #26，见 §11）

- **当前发布 = `device-test-kit` release**（2026-09-18 创建、**2026-09-26** 刷新；基线 `1.0.0-preview.24`；当前 kit #26 = 安全/性能第一批（#18）→ 启动/TLS/评审合规（#19）→ 最终热点回填（#20）→ headless abc 修复 + tester-run v6r2（#21）→ 设备里程碑回灌（#22）→ 工具刷新（#23）→ payload-in-libs 正式版（#24）→ 权限链 + Share/Scan 探测 + AOT 启动路径（#25）→ **P2-INTEROP 全量 `LibraryImport` hosting + TASK-MIG 打包任务程序集化 + PLAT-GAP 消费方缺省化（#26：hosting 桥 125 处 `LibraryImport`/`DllImport` 归零，导出契约 118/118；pack `tools/Microsoft.OpenHarmony.Tasks.dll` 6 任务程序集、`Hap.targets` 拆分 + `PlatformItems.targets`；AspNetCore KFR 固定 `11.0.0-rc.1.26425.128`、默认 `RuntimeIdentifier=openharmony-arm64`、`EnableAppHostPackDownload=false`；重建 hosting/Graphics/OpenHarmony 程序集与 bundle；`tester-run.sh` 仍为 v8；编号演进与内容见 §11）**）：整包大小/sha256、解压内容树 sha256、`.tar.gz.sha256` sidecar（边车）与探针 P1–P4 的数字**一律以 release 说明的「## Integrity」小节或 sidecar 为准**（`workload-latest` 镜像同值）——重签、预签或重新打包后必然变化，本页不写死任何哈希。
  kit #26 宿主 pack（构建产物）= **236,448 B** / `8fa24895…`（与 #25 逐字节相同；hap 内重签副本 240,544 B；导出契约 118/118）；hap 内 hosting DLL 由 47,616 B 增至 **55,808 B**（LibraryImport 重建）；`tester-run.sh` 仍为 **v8**（73,375 B，`script_version=8`，本轮未重传）。
...
- 当前 5 个 hap 实测（**快照示例，仅作参照，以随包文件与 release 说明为准**）：`bundleName=com.example.hellomauiapp`、26 波段 `minAPIVersion 50002014`、`targetAPIVersion 60101024`、`apiReleaseType Release`（api20 波段 `60000020`）、
  `compileSdkType HarmonyOS`、`compileSdkVersion 6.0.2.130`；`libs/arm64-v8a/` 含 14 个 `.so` = 12 个 .NET 运行时原生库（`libcoreclr`/`libclrjit`/`libhostfxr`/…）+ `libopenharmonyhost.so`（含 BATCH-1/2 Essentials 桥与加固）+ `libc++_shared.so`（kit #5 起该 in-kit 副本由 SDK ElfSigner 重签：`flags=0x10`、有效，此前为厂商 PKCS#7 签名，`ohos-workload 732766a`）；`dotnet.zip` 254 项且 0 个 `.so`（运行时原生库只随 hap `libs/<abi>/` 提供）；壳归档 `ets/modules.abc` 为加固壳（kit #14 快照 = 201,228 B 为历史；当前 kit #25/#26 = 234,620 B，五 hap 同值），自 kit #10 起入口 record 走非标准化 OHM URL（`useNormalizedOHMUrl=false`，`c2c4a9a`/`6e55ae6`），自 kit #11 起 abc 头为 `13.0.1.0`（`compatibleSdkVersion 18`，`95c89a7`/`ef1c947`）。文件大小与哈希以随包 `SHA256SUMS` 为准。
- 历史对照：本页早期与审计 §35 的 S 系列 kit/bundle 快照（含当时大小与哈希）均已被当前快照取代，不再复述。
- **包内 tester 文档不再写死 kit 哈希**：`快速开始.md`、`自签说明.md`、设备校验清单、`验收说明.md` 等一律指向 `device-test-kit` release 说明的「## Integrity」小节（`workload-latest` 镜像同值）或 `.tar.gz.sha256` sidecar；本页只锚定快照日期（2026-09-26），不记录当次快照数字。**注意 kit #26 包内 9 份 tester 文档为 kit #25 修订（仍写 #25；自 runtime-ohos 94c7730fcfb 导出）**：以 release 说明与仓库新交接文档 `docs/plans/2026-09-26-ohos-tester-handoff-kit26.md` 为准。
...
4. **发布面待办**：SDK release 上的 bundle 已随当前快照更新（`workload-1.0.0-preview.24`，重打包 30,501,359 B / `7b9ccd6e…`；sdk-ohos `eng/ohos-install/versions.env` 的 `WORKLOAD_BUNDLE_SHA256` 已锚定）；当前 kit #26 = P2-INTEROP 全量 `LibraryImport` hosting + TASK-MIG 打包任务程序集化 + PLAT-GAP 消费方缺省化（并在 #24 的 payload-in-libs：payload 随签名 hap `libs/` 原地启动 + `.dotnet-payload.json` marker、`dotnet.zip` 回退；宿主显式 W^X=0 + exec-memory 探针；`tester-run.sh` v8；含 #22 设备回灌、#23 工具刷新与 #25 权限/探测/AOT），见 §11 与 `docs/plans/2026-09-24-ohos-device-milestone.md`、`docs/plans/2026-09-25-ohos-tester-handoff-kit25.md`、`docs/plans/2026-09-26-ohos-tester-handoff-kit26.md`；
...
- **kit #14 与 297 基线（2026-09-23 上午；历史值——当前 kit #26、套件 315/floor 295，见 §11）**：`ohos-workload deaab95` 记录当时套件 **297** 条 / 0 Unhandled / 全部 perf `within=True`；
  当时 kit (#14) 解包实测五 hap 的 `ets/modules.abc` = 201,228 B（头 `13.0.1.0`）、`libs/arm64-v8a` 14 个 `.so`、
  `dotnet.zip` 253 项 0 `.so`（历史快照；当前 kit #26 为 254 项）、`sha256sum -c SHA256SUMS` 15/15 与 tree digest 校验 OK（数字只指向 release 说明/边车与交付物清单，不写入本页）。
...
- **当前基线与待办**：交互套件 **315** 条 / floor **295**（preflight 与 CI 同源解析 `[suite]` 行；`ohos-workload 92f7555`/`a10b73e`；本条写作时点的 297/≥277/≥226 为历史值）；
  2026-09-24 真机里程碑已达成（kit #18 + 测试方 5 项本地修复首次完整运行，见 §11 与 `docs/plans/2026-09-24-ohos-device-milestone.md`）；
  **stock kit（#22 起；#23 为同负载工具刷新、#24 为 payload-in-libs 正式版、#25 为权限链 + Share/Scan 探测 + AOT 启动路径、#26 为 P2-INTEROP/TASK-MIG/PLAT-GAP 收口）的复测仍待做**（自行重签；先看 §7 第 12 项的**三个**已修复分支），或回传 p7b + p12 + cer + keyAlias，
...
- **候选矩阵（历史口径，设备未执行；2026-09-24 真机成功未使用它们）**：**dynpkg**（`libIsolation` + abc host-binding record + 已确认入口形式）为
  ...
  **当前发布 = kit #26**（见下条），数字入口见 `docs/plans/2026-09-22-ohos-release-manifest.md`（更新快照），实验依据见
