...
> 评审整改（REV/RA/RB/RC-A..D）、真实缺陷修复（PI1）与 UX 深化（PJ1/PJ2）随 **kit #14** 出包；**当前发布 = kit #21**、
> 离设备套件 **315 条 / floor 295**（preflight 与 CI 同源），见 §4、§5、§10、§11）。
...
4. **发布面待办**：SDK release 上的 bundle 已随当前快照更新（`workload-1.0.0-preview.24`；当前 kit #21 = 安全/性能收口 + 启动与打包修复 + headless abc `13.0.1.0`，见 §11；
...
- **kit #14 与 297 基线（2026-09-23 上午；历史值——当前 kit #21、套件 315/floor 295，见 §11）**：`ohos-workload deaab95` 记录当时套件 **297** 条 / 0 Unhandled / 全部 perf `within=True`；
...
- **当前基线与待办**：交互套件 **315** 条 / floor **295**（preflight 与 CI 同源解析 `[suite]` 行；`ohos-workload 92f7555`/`a10b73e`；本条写作时点的 297/≥277/≥226 为历史值）；
  待测试方用 **kit #21**（当前发布，见 §11）重测（自行重签；先看 §7 第 12 项的**三个**已修复分支），或回传 p7b + p12 + cer + keyAlias，
...
- **候选矩阵（选一，设备跑批结果待回传）**：**dynpkg**（`libIsolation` + abc host-binding record + 已确认入口形式）为
  ...
  **当前发布 = kit #21**（见下条），数字入口见 `docs/plans/2026-09-22-ohos-release-manifest.md`（更新快照），实验依据见
  `docs/plans/2026-09-23-ohos-native-import-experiment.md` §8 与 `docs/plans/2026-09-23-ohos-napi-import-fix-playbook.md` §5。
- **kit #18 → #19 → #20 → #21（2026-09-23/24 发布，替代上文 kit #17 口径）**：kit #18 携带安全 + 性能第一批修复（A1/A2/A3、H-C1/H-C2、D-1..D-6、MB-1..MB-3、FIX-P1/P2）；kit #19 在其上追加 P17 启动解压跳过、H7 rawfile 描述符直读、H8/H11/H12、TLS 加固（H-C3 绝对路径 `dlopen` + H3 探测）、P12 增量签名、PR 评审合规整改与门禁加固（alloc 门禁 13,824 B、CI 缓存/PR 触发、`[suite]` 单一 floor 来源）；**kit #20** 收口最终热点回填（H10/P16/P9/P10/P19；见 `docs/plans/2026-09-23-ohos-performance-scan.md`）；**kit #21（当前）** 修复 headless 变体 abc（`24.0.0.0` → `13.0.1.0`，`ohos-workload 4e5491d`，随 workload bundle 与 pack 模板）并附带 tester-run v6r2。`device-test-kit` 与 bundle 的数字入口见 release `## Integrity` 与 `docs/plans/2026-09-22-ohos-release-manifest.md`（本页保持哈希无关）。
