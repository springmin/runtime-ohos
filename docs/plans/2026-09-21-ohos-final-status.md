...
> 评审整改（REV/RA/RB/RC-A..D）、真实缺陷修复（PI1）与 UX 深化（PJ1/PJ2）随 **kit #14** 出包；**当前发布 = kit #22**（见 §11 与
...
## 5. 发布物（当前 = `device-test-kit` release 说明的「## Integrity」小节，当前编号 kit #22，见 §11）

- **当前发布 = `device-test-kit` release**（2026-09-18 创建、**2026-09-24** 刷新；基线 `1.0.0-preview.24`；当前 kit #22 = 安全/性能第一批（#18）→ 启动/TLS/评审合规（#19）→ 最终热点回填（#20）→ headless abc 修复 + tester-run v6r2（#21）→ **设备里程碑回灌（#22：宿主按需 dlsym、`resources.index`、ZIP/mkdir、DevEco 工程布局；编号演进与内容见 §11）**）：整包大小/sha256、解压内容树 sha256、`.tar.gz.sha256` sidecar（边车）与探针 P1–P4 的数字**一律以 release 说明的「## Integrity」小节或 sidecar 为准**（`workload-latest` 镜像同值）——重签、预签或重新打包后必然变化，本页不写死任何哈希。
...
- `device-test-kit` release 创建于 2026-09-18；当前 **kit #22** 于 **2026-09-24** 刷新（tar.gz + sidecar 同批上传，同一 tar.gz 同步到 `workload-latest` 与 SDK release；具体时间与哈希见 release 说明「## Integrity」）。kit 编号演进：#7（22 日凌晨）→ #10（入口 record 修复）→ #11（abc `13.0.1.0`）→ #12（宿主 dlopen-only + 壳 `host` 守卫）→ … → #14（评审整改 + 真实缺陷修复 + UX 深化）→ #15/#16/#17（rawfile 资源桥 / 别名宿主 / RM1 `libIsolation`）→ #18（安全/性能第一批）→ #19（启动/TLS/评审合规）→ #20（最终热点回填）→ #21（headless abc `13.0.1.0` 修复 + tester-run v6r2）→ **#22（当前：设备里程碑回灌——宿主 `DT_NEEDED` 5 + 可选 API dlsym、`resources.index`（restool）、ZIP offset/mkdir、DevEco 工程布局；tester-run 仍 v6r2）**；探针 P1–P4（已随 preview.24 宿主重建）仍挂该 release。
...
4. **发布面待办**：SDK release 上的 bundle 已随当前快照更新（`workload-1.0.0-preview.24`；当前 kit #22 = 设备里程碑回灌（宿主按需 dlsym、`resources.index`、ZIP/mkdir、DevEco 工程布局），见 §11 与 `docs/plans/2026-09-24-ohos-device-milestone.md`；
...
- **kit #14 与 297 基线（2026-09-23 上午；历史值——当前 kit #22、套件 315/floor 295，见 §11）**：`ohos-workload deaab95` 记录当时套件 **297** 条 / 0 Unhandled / 全部 perf `within=True`；
...
- **当前基线与待办**：交互套件 **315** 条 / floor **295**（preflight 与 CI 同源解析 `[suite]` 行；`ohos-workload 92f7555`/`a10b73e`；本条写作时点的 297/≥277/≥226 为历史值）；
  ...
  **stock kit #22 的复测仍待做**（自行重签；先看 §7 第 12 项的**三个**已修复分支），或回传 p7b + p12 + cer + keyAlias，
...
- **候选矩阵（历史口径，设备未执行；2026-09-24 真机成功未使用它们）**：**dynpkg**（`libIsolation` + abc host-binding record + 已确认入口形式）为
  ...
  **当前发布 = kit #22**（见下条），数字入口见 `docs/plans/2026-09-22-ohos-release-manifest.md`（更新快照），实验依据见
