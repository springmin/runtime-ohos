...
归档 `tester-report-<时间戳>.tar.gz` 里有 `hilog/hilog-full.txt`、`hilog/hilog-filtered.txt`、`hilog/hilog-applib.txt`、`hilog/hilog-dlopen.txt`、`kmsg/`、`device/app-libs-arm64.txt`、`meta/kit-hap-sha256.txt`、`probes/`、`summary.txt`（含 `script_version`、`main_hap_sha256` 等字段）。截图/录屏与 `dotnet-status.txt` **不在**归档内，需人工另发；失败项请标注发生时间点。

`tester-run.sh` 当前为 **v6r2**（`script_version=6`）：`--tree-digest` 复用已校验摘要、明显更快（P16，实测每轮少读 163.5 MB 量级）；证据包新增 `meta/kit-hap-sha256.txt`/`main_hap_sha256`；行为与输出字段对旧调用兼容。
