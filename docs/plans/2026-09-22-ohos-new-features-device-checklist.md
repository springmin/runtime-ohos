...
`tester-run.sh` 当前为 **v8**（`script_version=8`；kit #24 版脚本 73,375 B / `6ca2093e…`，其余数字以 release 资产页「## Integrity」与随附 `gh api` 查询为准）：v6r2 的 `--tree-digest` 复用已校验摘要保持不变（P16，实测每轮少读 163.5 MB 量级）、证据包含 `meta/kit-hap-sha256.txt`/`main_hap_sha256`；v7 新增 bootstrap/rawfile 失败特征（`hilog-bootstrap.txt` + `bootstrap_errors`/`rawfile_errors`/`libload_errors`）、payload 状态（`payload-files`/`payload-marker` + `payload_present`/`payload_marker`）与 kit hap 自检（`meta/kit-selfcheck.txt` + `kit_index_ok`）；**v8（kit #24）再增 execmem 采集**（`hilog-execmem.txt` + `execmem_capture`/`execmem_lines`，kit 自检另加 `payload=yes|no`，判定见 `docs/plans/2026-09-24-ohos-tester-handoff-kit24.md`）。判定：`kit_index_ok=no` → 包早于 kit #22，换当前 kit 再测；`bootstrap_errors`/`rawfile_errors`>0 → 附 `hilog-bootstrap.txt` 回传（不影响退出码）；`payload_present=no` 在 kit #24 起属正常（payload 在 hap `libs/` 原地运行；该键只反映回退布局的 filesDir 解包，kit 自检 `payload=yes|no` 才是 marker 信号）。行为与输出字段对旧调用兼容。
